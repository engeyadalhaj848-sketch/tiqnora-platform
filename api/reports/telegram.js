...
export default async function handler(req, res) {
  ...
  if (cronSecret && req.headers.authorization !== `Bearer ${cronSecret}` && req.headers['x-cron-secret'] !== cronSecret) {
    return json(res, 401, { error: 'Unauthorized' });
  }
  try {
    const since = dayAgoIso();
    const [orders, leads, events, customers, aiConv, aiFailed, agents] = await Promise.all([
