...
- **Tiqnora Commerce AI**: طبقة دروبشيبنغ آمنة للموردين والمنتجات المرشحة وطلبات التنفيذ؛ يبدأ كل طلب مورد بـ«بانتظار اعتماد المالك».
...
2. نفّذ ملفات `supabase/migrations/` بالترتيب، وآخرها `009_commerce_ai_dropshipping.sql`
...
5. أضف `GEMINI_API_KEY` أو `OPENAI_API_KEY` أو `ANTHROPIC_API_KEY` كمتغير خادم في Vercel لتفعيل ردود الموظفين. عند استخدام Gemini يمكن ضبط `GEMINI_MODEL=gemini-3.6-flash` اختياريًا.
...
تكاملات AliExpress / Alibaba / Amazon / DSers لا تعتمد على مفاتيح داخل المتصفح أو قاعدة البيانات. بعد تفعيل حساب المورد الرسمي، تحفظ مفاتيحه في Vercel Environment Variables وتُنفذ الطلبات عبر endpoint خادمي محمي. لا يحق لوكيل Commerce AI الشراء أو النشر أو تعديل السعر تلقائيًا.
