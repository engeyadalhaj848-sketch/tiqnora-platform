...
export async function testConnection() {
  ...
  const expiryMs = conn.row.expires_at ? Date.parse(conn.row.expires_at) : 0;
  ...
  return {
    ok: conn.row.status === 'connected' && !expired,
    status: expired ? 'reauthorization_required' : (conn.row.status || 'connected'),
    provider: 'aliexpress',
    message: expired
      ? 'AliExpress authorization expired — reauthorization required.'
      : 'AliExpress OAuth connected; live dropshipping APIs enabled.',
    token_present: true,
...
export async function getShippingInfo(productId, opts = {}) {
  ...
  let r = await modernCall(
    'aliexpress.logistics.buyer.freight.calculate',
    freightParams,
    { requireToken: true }
  );
  let data = unwrapModern(r.data || {});
  let parsed = parseFreightResponse(data);
  ...
  if (!r.ok || !parsed.success) {
    ...
    const legacyParsed = parseFreightResponse(legacy.data || {});
    if (legacy.ok && legacyParsed.success) {
      r = legacy;
      parsed = legacyParsed;
    }
  }

  const valid = parsed.options.filter((x) => !x.error_desc && x.error_code !== 'PARAM_ERROR');
  const cheapest = valid.length
    ? [...valid].sort((a, b) => Number(a.amount || 0) - Number(b.amount || 0))[0]
    : null;

  return {
    ok: !!(r.ok && parsed.success && valid.length),
    provider: 'aliexpress',
    supplier_product_id: id,
    ...
    options: valid,
    error: parsed.error || (!valid.length ? 'No valid freight option returned' : null),
...
function parsePlaceOrderResponse(data) {
  const body = unwrapModern(data || {});
  const root = body?.aliexpress_trade_buy_placeorder_response || body?.aliexpressTradeBuyPlaceorderResponse || body;
  const result = root?.result || null;
  const rawOrders = result?.order_list?.number ?? result?.order_list ?? [];
  const orderIds = Array.isArray(rawOrders)
    ? rawOrders.map((x) => String(x))
    : rawOrders != null
      ? [String(rawOrders)]
      : [];
  return {
    success: result?.is_success === true || String(result?.is_success).toLowerCase() === 'true',
    order_ids: orderIds,
    error_code: result?.error_code || root?.error_response?.code || null,
    error_msg: result?.error_msg || root?.error_response?.msg || null,
    raw: body,
  };
}

/**
 * Creates an AliExpress supplier order only after explicit Tiqnora admin approval.
 * This does NOT pay the AliExpress order automatically.
 */
export async function placeApprovedOrder(payload = {}) {
  if (payload.approved !== true) {
    return {
      ok: false,
      status: 'approval_required',
      provider: 'aliexpress',
      message: 'Explicit admin approval is required before supplier order creation.',
      auto_purchase: false,
    };
  }
  ...
  const fullName = String(a.full_name || a.name || a.contact_person || '').trim();
  const mobile = String(a.mobile_no || a.phone || '').trim();

  if (!address || !city || !fullName || !mobile || !itemsIn.length) {
    return {
      ok: false,
      status: 'invalid_payload',
      provider: 'aliexpress',
      message: 'address, city, full_name, mobile_no and items are required',
      auto_purchase: false,
    };
  }
  ...
  for (const item of itemsIn) {
    ...
    const qty = Math.max(1, Math.min(99, Number(item.product_count || item.quantity || 1) || 1));
    if (!/^\d+$/.test(productId)) {
      return {
        ok: false,
        status: 'invalid_payload',
        provider: 'aliexpress',
        message: 'Each AliExpress item requires a numeric product_id',
        auto_purchase: false,
      };
    }
    const row = {
    ...
    if (item.sku_attr != null && String(item.sku_attr).trim()) row.sku_attr = String(item.sku_attr).trim();
    if (item.logistics_service_name) row.logistics_service_name = String(item.logistics_service_name).slice(0, 120);
    if (item.order_memo) row.order_memo = String(item.order_memo).slice(0, 500);
    productItems.push(row);
  }

  const logisticsAddress = {
    address: address.slice(0, 500),
    address2: String(a.address2 || '').slice(0, 500),
    city: city.slice(0, 120),
    contact_person: fullName.slice(0, 120),
    country: String(a.country || 'SA').toUpperCase().slice(0, 2),
    full_name: fullName.slice(0, 120),
    locale: String(a.locale || 'en_US').slice(0, 12),
    mobile_no: mobile.slice(0, 40),
    phone_country: String(a.phone_country || '+966').slice(0, 8),
    province: String(a.province || a.city || '').slice(0, 120),
    zip: String(a.zip || a.postal_code || '').slice(0, 24),
  };
  ...
  let r = await modernCall('aliexpress.trade.buy.placeorder', params, { requireToken: true });
  let parsed = parsePlaceOrderResponse(r.data);

  // New international apps normally use api-sg; keep old TOP only as compatibility fallback.
  if (!r.ok || !parsed.success) {
    const legacy = await legacyCall('aliexpress.trade.buy.placeorder', params, { requireToken: true });
    const lp = parsePlaceOrderResponse(legacy.data);
    if (legacy.ok && lp.success) {
      r = legacy;
      parsed = lp;
    }
  }

  if (!r.ok || !parsed.success || !parsed.order_ids.length) {
    return {
      ok: false,
      status: 'error',
      provider: 'aliexpress',
      message: parsed.error_msg || 'AliExpress order creation failed',
  ...
  return {
    ...
    status: 'submitted',
    provider: 'aliexpress',
    external_order_ids: parsed.order_ids,
    external_order_id: parsed.order_ids[0] || null,
    transport: r.transport || null,
