...
export async function auditProductPipeline(supplierProductId, { countryCode = 'SA' } = {}) {
  ...
  // Prefer in-stock + freight, else freight-only, else first
  const withFreight = audited.filter((a) => a.shipping_cost_source === 'cj_live_freight');
  const primary =
    publishable[0] ||
    audited.find((a) => (a.total_available_inventory || 0) > 0) ||
    withFreight[0] ||
    audited[0] ||
    null;
