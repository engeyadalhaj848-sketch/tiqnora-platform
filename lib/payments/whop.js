...
export function getWhopConfig() {
  ...
  return {
    sandbox, apiKey, accountId, webhookSecret, currency,
...
async function whopFetch(path, { method = 'GET', body } = {}) {
  ...
  if (!cfg.apiKey) return { ok: false, status: 503, error: 'WHOP_API_KEY missing', data: null };
  const headers = {
  ...
  if (!res.ok) return { ok: false, status: res.status, error: data?.message || data?.error || `Whop HTTP ${res.status}`, data };
  return { ok: true, status: res.status, data };
}

export async function createCheckoutConfiguration({ amount, currency, orderId, orderNumber, title = 'Tiqnora Order' }) {
  const cfg = getWhopConfig();
  if (!cfg.configured) return { ok: false, status: 'not_configured', message: 'Whop credentials not configured', missing: cfg.missing };
  const cur = (currency || cfg.currency || 'usd').toLowerCase();
  const price = Number(amount);
  if (!Number.isFinite(price) || price <= 0) return { ok: false, status: 'error', message: 'invalid amount' };
  const body = {
    account_id: cfg.accountId,
    plan: { initial_price: price, plan_type: 'one_time', currency: cur, title: String(title).slice(0, 80) },
    metadata: { order_id: String(orderId), order_number: String(orderNumber || ''), source: 'tiqnora', environment: cfg.sandbox ? 'sandbox' : 'production' },
  };
  const result = await whopFetch('/checkout_configurations', { method: 'POST', body });
  if (!result.ok) return { ok: false, status: 'error', message: result.error, details: result.data };
  const sessionId = result.data?.id || null;
  const planId = result.data?.plan?.id || result.data?.plan_id || null;
  if (!sessionId) return { ok: false, status: 'error', message: 'Whop response missing checkout id' };
  return { ok: true, sessionId, planId, currency: cur, amount: price, sandbox: cfg.sandbox, environment: cfg.sandbox ? 'sandbox' : 'production' };
}
...
export function verifyWebhookSignature(rawBody, headers) {
  ...
  if (Number.isFinite(ts) && Math.abs(Date.now() / 1000 - ts) > 300) return { ok: false, reason: 'timestamp_out_of_range' };
  ...
  const providedList = String(signatureHeader).split(' ').map((p) => p.replace(/^v1,/i, '').trim()).filter(Boolean);
