...
export function isSandbox() {
  const env = String(process.env.WHOP_ENVIRONMENT || 'sandbox').trim().toLowerCase();
...
async function whopFetch(path, { method = 'GET', body } = {}) {
  ...
  // Safe, minimal diagnostics only (no secrets / no key material)
  if (process.env.WHOP_DEBUG === '1') {
...
/**
 * Safe production diagnostics for Whop credentials/resources.
 * Returns only status/boolean information — never secrets or resource IDs.
 */
export async function diagnoseWhopAccess() {
  const cfg = getWhopConfig();
  const out = {
    environment: cfg.sandbox ? 'sandbox' : 'production',
    ...
    checkout_configurations_read_status: null,
    payments_read_status: null,
    product_read_status: null,
    product_company_match: null,
  };
  if (!cfg.configured) return out;

  try {
    const c = await whopFetch(`/checkout_configurations?account_id=${encodeURIComponent(cfg.accountId)}&first=1`);
    out.checkout_configurations_read_status = c.status;
  } catch {
    out.checkout_configurations_read_status = 0;
  }

  try {
    const p = await whopFetch(`/payments?company_id=${encodeURIComponent(cfg.accountId)}&first=1`);
    out.payments_read_status = p.status;
  } catch {
    out.payments_read_status = 0;
  }

  if (cfg.productId) {
    try {
      const p = await whopFetch(`/products/${encodeURIComponent(cfg.productId)}`);
      out.product_read_status = p.status;
      if (p.ok && p.data && typeof p.data === 'object') {
        const owner =
          ...
          p.data.account?.id ||
          null;
        out.product_company_match = owner ? String(owner) === cfg.accountId : null;
      }
    } catch {
      out.product_read_status = 0;
    }
  }
...
export async function createCheckoutConfiguration({
  ...
  if (!cfg.productId) {
    return {
      ok: false,
      status: 'error',
      message: 'WHOP_PRODUCT_ID missing',
      details: { environment: cfg.sandbox ? 'sandbox' : 'production' },
    };
  }

  // Follow Whop's current reference flow:
  // 1) create a one-time plan attached to the existing Tiqnora product
  // 2) create a checkout configuration from that plan_id
  // This avoids the inline-plan authorization path that returned 403 in Production.
  const planBody = {
    company_id: cfg.accountId,
    ...
    internal_notes: `Tiqnora order ${String(orderNumber || orderId).slice(0, 80)}`,
    unlimited_stock: true,
  };

  const planResult = await whopFetch('/plans', { method: 'POST', body: planBody });
  if (!planResult.ok) {
    return {
      ok: false,
      status: 'error',
      message: planResult.parsed?.message || planResult.error,
      code: planResult.parsed?.code || null,
      http_status: planResult.status,
      permission_hint: {
        required_permissions: ['plan:create', 'access_pass:basic:read', 'plan:basic:read'],
      },
      details: {
        stage: 'plan_create',
        has_product_id: true,
        environment: cfg.sandbox ? 'sandbox' : 'production',
        currency: cur,
        whop_error: planResult.parsed?.code || planResult.parsed?.message || null,
      },
    };
  }

  const planId = planResult.data?.id || null;
  if (!planId) {
    return {
      ok: false,
      status: 'error',
      message: 'Whop response missing plan id',
      details: { stage: 'plan_create' },
    };
  }
  ...
  const result = await whopFetch('/checkout_configurations', { method: 'POST', body: checkoutBody });
  if (!result.ok) {
    return {
      ok: false,
      status: 'error',
      ...
      permission_hint: {
        required_permissions: ['checkout_configuration:create', 'checkout_configuration:basic:read'],
      },
      details: {
        stage: 'checkout_configuration_create',
        has_product_id: true,
        environment: cfg.sandbox ? 'sandbox' : 'production',
        currency: cur,
