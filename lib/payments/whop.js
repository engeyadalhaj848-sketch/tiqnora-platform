...
/**
 * Whop uses Standard Webhooks.
 * Signed content: "{webhook-id}.{webhook-timestamp}.{rawBody}"
 * Signature header: "v1,<base64>" (space-separated list allowed)
 * Secret: pass WHOP_WEBHOOK_SECRET exactly as issued (ws_... preferred).
 * Docs: do not JSON.parse/stringify body before verify — raw bytes/text only.
 */
function deriveWebhookKeyCandidates(secret) {
  const out = [];
  const push = (buf) => {
    if (!buf || !buf.length) return;
    // de-dupe by hex
    const hx = buf.toString('hex');
    if (out.some((b) => b.toString('hex') === hx)) return;
    out.push(buf);
  };
  // 1) Full secret as UTF-8 (Whop: use ws_ string; helper derives — we also try raw)
  push(Buffer.from(secret, 'utf8'));
  if (secret.startsWith('ws_')) {
    const rest = secret.slice(3);
    push(Buffer.from(rest, 'utf8'));
    // hex material after ws_ (common dashboard format)
    if (/^[0-9a-fA-F]+$/.test(rest) && rest.length % 2 === 0) {
      try { push(Buffer.from(rest, 'hex')); } catch { /* */ }
    }
    // base64 material after ws_
    try {
      const b = Buffer.from(rest, 'base64');
      if (b.length >= 16) push(b);
    } catch { /* */ }
  }
...
export function verifyWebhookSignature(rawBody, headers) {
  ...
  if (typeof rawBody !== 'string') {
    return { ok: false, reason: 'raw_body_must_be_string' };
  }
  ...
  // Also accept svix-* aliases some gateways forward
  const id = String(h['webhook-id'] || h['svix-id'] || '');
  const timestamp = String(h['webhook-timestamp'] || h['svix-timestamp'] || '');
  const signatureHeader = String(h['webhook-signature'] || h['svix-signature'] || '');
  ...
  // 5-minute replay window (Whop / Standard Webhooks)
  if (Math.abs(Date.now() / 1000 - ts) > 300) {
    return { ok: false, reason: 'timestamp_out_of_range' };
  }

  const signedContent = `${id}.${timestamp}.${rawBody}`;
  const keys = deriveWebhookKeyCandidates(secret);
  const providedList = signatureHeader
    .split(/\s+/)
    .map((p) => p.replace(/^v1[,=]/i, '').trim())
    .filter(Boolean);

  for (const keyBuf of keys) {
    const computed = createHmac('sha256', keyBuf).update(signedContent, 'utf8').digest('base64');
