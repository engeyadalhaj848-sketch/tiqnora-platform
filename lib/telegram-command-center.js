...
function helpText(agents) {
  return [
    ...
    '/daily — تجهيز وتشغيل مهام الوكلاء اليومية',
    '/report — تقرير فوري',
    '/help — هذه المساعدة',
    '',
...
async function processCommand(message, session, agents) {
  ...
  if (command === '/daily') {
    ...
    const result = await runQueuedTasks({ limit: 12 });
    await sendTelegramText(chatId, [
      '🚀 تم تشغيل فريق Tiqnora اليومي.',
      `الوكلاء المفعّلون: ${prepared.agents_enabled?.length || 0}`,
      `مهام جديدة: ${prepared.tasks_created || 0}`,
      `مهام موجودة مسبقاً: ${prepared.tasks_existing || 0}`,
      `تمت: ${result.completed || 0}`,
      `فشلت: ${result.failed || 0}`,
      '',
      'لن يتم النشر أو مراسلة العملاء تلقائياً قبل موافقتك.'
...
function naturalControlIntent(text) {
  const value = String(text || '').trim().toLowerCase();
  if (!value) return null;

  if (/(حالة|وضع|ستاتس).*(المنصة|النظام|تيكنورا|tiqnora)|(كيف|وش|ايش).*(وضع|حالة).*(المنصة|النظام)/i.test(value)) return 'status';
  if (/(اعطني|ارسل|جيب|وريني|اعرض).*(تقرير)|(التقرير).*(اليومي|اليوم|المنصة|النظام)/i.test(value)) return 'report';
  if (/(اعرض|وريني|مين|ماهي|ما هي).*(الوكلاء|الاجنتس|agents)|(الوكلاء|الاجنتس).*(النشط|المتاح|موجود)/i.test(value)) return 'agents';
  if (/(اعرض|وريني|هات|جيب).*(الفرص|العملاء المحتملين|prospects)|(فرص المبيعات|العملاء المحتملين)/i.test(value)) return 'prospects';
  if (/(اعرض|وريني|هات|جيب).*(المهام|tasks)|(المهام).*(المفتوحة|المتبقية|الحالية)/i.test(value)) return 'tasks';
  if (/(شغل|شغّل|فعل|فعّل|ابدأ).*(الوكلاء|الاجنتس|فريق العمل)|(الوكلاء|الاجنتس|فريق العمل).*(شغل|شغّل|فعل|فعّل|ابدأ)/i.test(value)) return 'daily';
  if (/(نفذ|نفّذ|شغل|شغّل|ابدأ).*(المهام|مهام الوكلاء)|(المهام).*(نفذ|شغل|ابدأ)/i.test(value)) return 'run';
  if (/(المساعدة|ساعدني|الأوامر|الاوامر|help)/i.test(value)) return 'help';
  return null;
}

async function processNaturalControl(message, agents) {
  const intent = naturalControlIntent(message.text);
  if (!intent) return false;

  const chatId = String(message.chat.id);
  const messageId = message.message_id;

  if (intent === 'status' || intent === 'report') {
    await typing(chatId);
    await sendTelegramText(chatId, await statusText(), { reply_to_message_id: messageId });
    return true;
  }
...
export async function handleTelegramUpdate(update) {
  ...
  const naturalHandled = await processNaturalControl(message, agents);
  if (naturalHandled) return { ok: true, natural_command: true };
