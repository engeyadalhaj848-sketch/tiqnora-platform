...
export function isTelegramConfigured() {
  return Boolean(botToken() && allowedChats().size);
}
...
async function statusText() {
  const [prospects, tasks, agents, integrations] = await Promise.all([
  ...
  return [
    ...
    `📨 Telegram: ${isTelegramConfigured() ? 'مُعدّ' : 'ينقص Token/Chat ID'}`
  ].join('\n');
}
