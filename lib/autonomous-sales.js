...
async function callGemini({ prompt, system = '', grounded = false, maxOutputTokens = 8192 }) {
  ...
  if (!response.ok) {
    ...
    if (response.status === 429) error.code = 'AI_PROVIDER_QUOTA';
    else if ([500, 502, 503, 504].includes(response.status)) error.code = 'AI_PROVIDER_UNAVAILABLE';
    else error.code = 'AI_PROVIDER_ERROR';
    throw error;
  }
...
export async function discoverProspects() {
  ...
  try {
    ...
    if (['AI_PROVIDER_QUOTA', 'AI_PROVIDER_UNAVAILABLE', 'AI_PROVIDER_NOT_CONFIGURED'].includes(error.code)) {
      return {
        degraded: true,
...
export async function runAutonomousGrowth() {
  const prospecting = await discoverProspects().catch(error => ({
    degraded: true,
    candidates: 0,
    saved: 0,
    skipped: 0,
    failed: 0,
    results: [],
    error_code: error.code || 'PROSPECTING_FAILED',
    error_message: String(error.message || error).slice(0, 500)
  }));
