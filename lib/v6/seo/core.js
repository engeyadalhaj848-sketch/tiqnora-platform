...
const TIQNORA_VERCEL_HOST =
  /^(?:tiqnora|tiqnora-ai)(?:-[a-z0-9-]+)?-engeyadalhaj848-7848\.vercel\.app$/i;
...
export function isAllowedAuditUrl(url) {
  try {
    ...
    // Vercel previews for this Tiqnora project/account only.
    if (u.protocol === 'https:' && TIQNORA_VERCEL_HOST.test(u.hostname)) return true;
    return false;
  } catch {
    return false;
  }
