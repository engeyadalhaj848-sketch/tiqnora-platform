...
function mergedLeadCustomFields(existing, analysis, enrichment) {
...
async function createOrUpdateLead(rest, organizationId, event, analysis, enrichment, contactId) {
  ...
  if (matched) {
    ...
    const patch = withoutNulls({
      ...
      ...(salesLike ? {
        ...
        custom_fields: mergedLeadCustomFields(matched, analysis, enrichment)
      } : {})
    });
  ...
  const payload = {
    ...
    last_contact_at: now,
    custom_fields: mergedLeadCustomFields(null, analysis, enrichment)
  };
...
async function upsertConversation(rest, organizationId, event, inboxEvent, analysis, enrichment, contactId, leadId) {
...
async function insertMessage(rest, organizationId, storedEvent, event, inboxEvent, analysis, enrichment, conversationId) {
...
export async function planSocialCrmEvent(event = {}, options = {}) {
  ...
  return { skipped: false, inboxEvent, analysis, enrichment };
}
