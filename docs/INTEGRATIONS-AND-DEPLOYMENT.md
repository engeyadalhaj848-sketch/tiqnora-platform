...
6. Connect Meta, WhatsApp Cloud and TikTok through official OAuth flows; never paste tokens into content records.
   OAuth entry points are `/api/social/oauth/meta`, `/api/social/oauth/whatsapp`, `/api/social/oauth/tiktok`, and `/api/social/oauth/linkedin`. Tokens are exchanged server-side and encrypted in `social_provider_tokens`.
...
- Set `OAUTH_STATE_SECRET` and a base64-encoded 32-byte `SOCIAL_TOKEN_ENCRYPTION_KEY` in Vercel before enabling OAuth. Apply migration `008_social_oauth_tokens.sql`.
