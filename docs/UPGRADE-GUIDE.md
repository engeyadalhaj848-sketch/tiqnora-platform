...
- نفّذ أيضًا `supabase/migrations/002_security_hardening.sql` إذا كانت قاعدة البيانات أُنشئت قبل إضافة الحماية الأخيرة.
- لا تحفظ مفاتيح شركات الشحن أو الدفع أو مزودي AI داخل `site_settings` أو أي إعداد عام يُقرأ من المتصفح.
- مفاتيح API السرية يجب أن تبقى في متغيرات خادم/Edge Function مشفرة؛ لوحة التحكم تخزن فقط أسماء المزودين والإعدادات غير السرية.
- الطلبات العامة تبدأ دائمًا بحالة `pending / unpaid`، ولا تتحول إلى مدفوعة إلا من webhook موثوق.
